...
// console.log(parts)
var [seedToSoil, soilToFertilizer, fertilizerToWater, waterToLight, lightToTemperature, temperatureToHumidity, humidityToLocation,] = parts.slice(1, parts.length).map(lookup => lookup.split(/:/)[1].trim().split(/\n/).map(row => row.split(" ").map(entry => parseInt(entry))));
var seedList = parts[0].split(/:/)[1].trim().split(" ").map(seed => parseInt(seed));
if (false) {
    console.log(seedList);
    console.log("\n", seedToSoil, "\n", soilToFertilizer, "\n", fertilizerToWater, "\n", waterToLight, "\n", lightToTemperature, "\n", temperatureToHumidity, "\n", humidityToLocation, "\n");
}
function lookUpX(search, lookup) {
...
function walkLookups(search, lookupsArr = lookups) {
...
// NOTE p2 start
function sliding(arr, size, offset) {
    var arrArr = [];
